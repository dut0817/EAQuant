...
def can_smooth_fc_fc(fc1, fc2, scales):
    return (
        fc1.out_features == fc2.in_features
        and scales.numel() == fc1.out_features
    )

def can_smooth_q_k(q_proj, k_proj, scales):
    return (
...
def smooth_and_quant_temporary(model, args, isllama):
    ...
    if args.let:
        ...
        if isllama:
            ...
            if can_smooth_fc_fc(model.self_attn.v_proj, model.self_attn.o_proj, model.out_smooth_scale):
                smooth_fc_fc_temporary(model.self_attn.v_proj,model.self_attn.o_proj,
                                    model.out_smooth_scale, model.out_smooth_shift)
            if can_smooth_q_k(model.self_attn.q_proj, model.self_attn.k_proj, model.qkt_smooth_scale):
