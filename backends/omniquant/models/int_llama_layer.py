...
def _can_smooth_fc_fc(fc1, fc2, scales):
    return (
        fc1.out_features == fc2.in_features
        and scales.numel() == fc1.out_features
    )


def _can_smooth_q_k(q_proj, k_proj, scales):
    return (
        q_proj.out_features == k_proj.out_features
        and scales.numel() == q_proj.out_features
    )
...
class QuantLlamaAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        self.rotary_uses_position_ids = self._rotary_uses_position_ids()
    ...
    def _rotary_uses_position_ids(self):
        try:
            return "position_ids" in inspect.signature(self.rotary_emb.forward).parameters
        except (TypeError, ValueError):
            return False

    def _apply_rotary_pos_emb(self, query_states, key_states, cos, sin, position_ids):
        try:
    ...
    def forward(
        ...
        **kwargs,
        ...
        is_transformers_cache = hasattr(past_key_value, "update")
        if past_key_value is not None and not is_transformers_cache:
        ...
        if position_embeddings is None:
            ...
            if self.rotary_uses_position_ids:
        ...
        if is_transformers_cache:
            if self.layer_idx is None:
                raise ValueError("Caching requires `layer_idx` on QuantLlamaAttention.")
        ...
        if use_cache:
            past_key_value = past_key_value if is_transformers_cache else (key_states, value_states)
        else:
            past_key_value = None
...
class QuantLlamaDecoderLayer(nn.Module):
    ...
    def smooth_and_quant_temporary(self):
        if self.let:
            ...
            if _can_smooth_fc_fc(self.self_attn.v_proj, self.self_attn.o_proj, self.out_smooth_scale):
                smooth_fc_fc_temporary(self.self_attn.v_proj,self.self_attn.o_proj,
                                    self.out_smooth_scale, self.out_smooth_shift)
            if _can_smooth_q_k(self.self_attn.q_proj, self.self_attn.k_proj, self.qkt_smooth_scale):
