...
def _load_tokenizer(model_name, hf_token=None):
...
def _validate_rotate_down_dim(config, args):
    if args.online_hadamard not in ("all", "down"):
        return
    if args.rotate_down_dim < 1:
        raise ValueError("rotate_down_dim must be >= 1")
    if config.intermediate_size % args.rotate_down_dim != 0:
        raise ValueError(
            f"rotate_down_dim={args.rotate_down_dim} must divide intermediate_size={config.intermediate_size}"
        )
    try:
        hadamard_utils.get_hadK(config.intermediate_size // args.rotate_down_dim)
    except AssertionError as exc:
        raise ValueError(
            f"rotate_down_dim={args.rotate_down_dim} is incompatible with intermediate_size={config.intermediate_size} "
            "for the online down Hadamard path"
        ) from exc
...
def get_model(
    ...
    if "qwen" in model_name_lower:
        ...
        _validate_rotate_down_dim(config, args)
        ...
        config = transformers.MistralConfig.from_pretrained(model_name)
        ...
        raise ValueError(
            f"Unsupported model {model_name}. OSTQuant currently supports Llama/OpenBioLLM, Qwen2-style, and Mistral-style models."
        )
    ...
    def _rotate_layer_inplace(self,model,layer:QuantDecoderLayer,args):
        ...
        if args.rotate_ov:
            ...
            if v.bias is not None:
                ...
                v.bias.data = (v.bias.reshape(num_kv_heads,head_dim).unsqueeze(1).to(RD) @ R_ov.to(RD)).squeeze(1).reshape(v.bias.shape)
