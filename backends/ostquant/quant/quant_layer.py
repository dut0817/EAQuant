...
def build_rotary_matrix(cos, sin):
    ...
    weight[:, : head_dim // 2] += sin_diag[:, head_dim // 2 :]
    return weight.reshape(*leading_shape, head_dim, head_dim)
...
class QuantAttention(nn.Module):
    def __init__(self, org_module: LlamaAttention, config: LlamaConfig, args):
        ...
        self.is_qwen2 = getattr(config, "model_type", "") == "qwen2"
    ...
    def forward(
        ...
        if self.is_qwen2:
            ...
            cos_cache, sin_cache = self.rotary_emb(value_states, seq_len=kv_seq_len)
        ...
        if past_key_value is not None:
            ...
            if self.is_qwen2:
                cache_kwargs["sin"] = sin_cache
                cache_kwargs["cos"] = cos_cache
