...
def fuse_layer_norms(model): 
    ...
    for layer in layers:
        ...
        if model_type in (model_utils.LLAMA_MODEL, model_utils.QWEN_MODEL, model_utils.MISTRAL_MODEL):
    ...
    if model_type == model_utils.OPT_MODEL:
        ...
        replace_type = transformers.models.mistral.modeling_mistral.MistralRMSNorm
...
def rotate_attention_output(layer, Q, model_type) -> None:
    ...
    elif model_type in (model_utils.LLAMA_MODEL, model_utils.QWEN_MODEL, model_utils.MISTRAL_MODEL):
...
def rotate_faster_down_proj(layer, model_type, hardK):
    ...
    if model_type in (model_utils.LLAMA_MODEL, model_utils.QWEN_MODEL, model_utils.MISTRAL_MODEL):
        ...
        raise ValueError(f'Faster MLP is only supported for llama-like models!')
...
def rotate_mlp_up_down(layer,T:torch.Tensor,model_type:model_utils.LLAMA_MODEL):
    assert model_type in (model_utils.LLAMA_MODEL, model_utils.QWEN_MODEL, model_utils.MISTRAL_MODEL)
