...
from eaquant.data.medmix import get_medmix


def _build_tokenizer(model, hf_token=None, use_fast=True):
    tokenizer_kwargs = {}
...
def get_wikitext2(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
    ...
    tokenizer = _build_tokenizer(model, hf_token=hf_token, use_fast=True)
...
def get_c4_new(nsamples, seed, seqlen, model, hf_token=None, eval_mode=False):
    ...
    tokenizer = _build_tokenizer(model, hf_token=hf_token, use_fast=False)
...
def get_loaders(
    ...
    if os.path.exists(cache_f):
        ...
        if 'medmix' in name:
            loader = get_medmix(nsamples, seed, seqlen, model, hf_token, eval_mode)
